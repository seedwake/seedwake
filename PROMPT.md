...
```
telegram:100200300 (Alice) [msg:305] 引用了我之前说的 [msg:298]："好，我自己找一篇关于有氧锻炼的文章"
说：谢谢你
```

**c) 用户昵称缺失。** 当前只展示 `telegram:100200300`，模型不知道这个人叫什么。应把 Telegram 的 first_name/username 传入，展示为类似 `telegram:100200300 (Alice)` 的格式，让模型在回应时能用对方的名字。
...
与 [Jam](telegram:100200300) 的近期对话：
...
**原则**：
- 使用 `core.common_types.elapsed_ms` 统一计时
...
退化检测触发深睡，但深睡只做归档和重启，不改变 context window 内的念头、笔记、对话历史、没有新刺激。重启后拿到几乎相同的 prompt，大概率继续输出一样的东西。

退化检测不应该触发深睡作为"解决方案"——深睡解决的是存储问题，不是思维停滞问题。退化时应该：
- 触发更强的停滞警告（已有，但来得太晚）
- 强制要求至少一个 action（当前只是建议）
- 或者直接在停滞警告中注入一个具体的行动建议（如"去查一下天气"或"去读点新闻"）
