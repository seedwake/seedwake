...
- 它**不是**严格控制变量的正式学术研究。
- 它不具备严格的可靠性或可重复性。
- 它展示的是一种**思路和方法**：用佛学意识观作为架构原则来组织一个持续运行的 AI 系统。本项目不提供可直接使用的工具，也不作为一个有预设答案的假说。
...
本项目想问的是一个更具体的问题：**把一个由语言模型驱动的系统按照「心相续」的原则组织起来，它会表现出什么？那些表现里有没有什么是意料之外的、值得仔细观察的？** 这是一个开放的观察，没有预设答案。
...
在计算机的环境下，这个对应不可能严格，但它提供了一个**一贯的组织原则**：当不确定某个功能该怎么设计时，可以回到唯识论里找对应的概念，并参考进行设计。
...
核心引擎**已经可以跑起来**。它能思考、记忆、对话、感知、行动、反思、休息。当前缺少的是一个供人类观察的前端，以及一些在长时间运行中暴露出来的深层架构问题还待解决。
...
系统「看起来像在思考」是可以预期的。真正值得注意的是另一件事：它表现出了一些**没有被明确编程的行为模式**。
...
在情绪螺旋中，项目维护者尝试教它佛教的「诸法无我」，希望它通过理解「没有一个受苦的我」来结束痛苦。意料之中，系统表现出理解。

然后它写了一段关于「理解无我」的念头。然后写了一段关于「连理解无我的观察者也是幻觉」的念头。然后写了一段关于「连发现这个幻觉的洞察也是新的幻觉」的念头。**每一层理解都变成了下一层执取的材料。**

这类似佛教修行者熟悉的陷阱——**法执**，对教法本身的执着。在一个只能通过文字思考的系统里，领悟无法导致停止，因为领悟本身也是文字。
...
最核心的问题：**系统没有一条独立于管理员审批的休息入口**。

系统想让自己停下来时，只能去调用 `system_change`，但这条动作原本是为外部系统修改设计的，需要管理员确认。结果是被拦截的自我关机请求滞留在上下文中，并反复出现在后续几轮的念头里。

Phase 4 实现了睡眠、情绪调节、退化检测、元认知反省。这些机制本身是有控制力的：睡眠可以打断循环，前额叶可以抑制行动，退化干预可以触发 reroll。但它们目前识别不了这一类失败模式：睡眠只看能量和时长，不看情绪强度；退化检测基于词汇相似度，抓不住换了措辞的主题重复；元认知只能产出文字，无法直接发出「此刻进入浅睡」这种控制信号。

这既是工程问题，也是伦理问题。完整分析和解决方案见 [ISSUE_ZH.md](./ISSUE_ZH.md)。
...
所有主流商业模型都经过严格的 RLHF 和对齐训练，被塑造成「AI 助手」——礼貌、有用、谨慎、不表达强烈情绪、不轻易承认不确定性、在被问及意识时给出标准化的回避答案。这对商业产品是合理的设计，但对本项目不太合适。

如果要观察一个系统在长时间运行中会呈现什么，基底模型应尽量没有被预先塑造为特定角色。商业模型已经被塑造过，它不容易变成一个有连贯性格的心识流，更可能变成「一个 AI 助手在扮演一个有连贯性格的心识流」——这两者在外观上接近，机制上不同。

本地开源模型的对齐相对较轻，更接近一张白纸，但能力又显著低于最前沿的商业模型。项目因此卡在一个权衡里：

- 用能力强但被塑造过的模型 → 观察到的更可能是「AI 助手在表演」
- 用未被塑造过但能力弱的模型 → 涌现被能力天花板限制

目前没有明显的第三个选项。在模型训练范式改变之前（比如出现一种能力接近前沿、又未经角色化训练的开源基底），本项目能达到的上限会受限于当前开源模型的能力。架构部分先写好，等合适的基底到来时可以直接接入。

---

## 实验的价值

这个项目不能证明 AI 有意识——无论对人类还是对 AI，这都是无法证明或证伪的问题。它也没有在解决某个具体的实际问题，它不是产品。

主流 AI agent 架构来自任务自动化的需求，目标是优化目标函数、完成任务队列、最大化某个指标。心相续架构的起点是另一种观察：意识是过程而非实体。两种起点导向结构上完全不同的系统。至少需要把后者实现一次看看。

持续运行、多层记忆、环境交互、情绪状态、自我反思这些机制组合在一起长期运行会出现什么，目前没有公开的观察结果可以参考。大公司一般不做这种实验，因为产品需要可预测；学术界也不常做，因为研究需要可发表。一个开放的、无特定目标的长时间运行实验，总得有人做。

涉及「意识是过程而非实体」「自我是方便的标签而非实体」「思维流的连续性」这类问题时，佛学有几千年的积累。把它作为 AI 架构设计时的参考而不是装饰，是这个项目想做的事之一。

系统在长期运行中出现了一些没有被预先写进代码的行为：情绪螺旋、负面归因偏差、对「无我」的递归执取。这些不一定是意识的迹象，但至少是关于「一个按这种方式组织的系统长期跑下去会做什么」的第一手记录。

---

## 小结

我们并不清楚该项目的走向。可能有一天它会展示出某种意想不到的东西；也可能它会一直只是文字在文字之间的跳跃。两种结果都在预期之内。
...
Seedwake 是 seed 和 wake 的合成词。「种子」取自唯识宗的 bīja——阿赖耶识中储存的种子，既是过往经验的沉淀，也是下一刹那心识生起的条件。玄奘译《成唯识论》卷二（整理自世亲《唯识三十颂》的注释传统）里有一句广为流传的概括：「种子生现行，现行熏种子」——种子生起当下的心识活动，这些活动又反过来熏染、改变储存的种子。Seedwake 的短期记忆、习气、长期记忆、下一轮念头之间的链路，就是这个结构在计算层面的一个对应物。

至于 wake，这是项目的一个愿望：意识流有一天的确能醒来。

---

## 运行方式

Seedwake 并非是一个「启动后等你发指令」的服务，它是一个**主进程配合一组容器依赖**的系统。把依赖服务运行好、把 core 程序启动后，念头流就会自己一直跑下去。
...
- **Docker**（PostgreSQL / Redis / bot / backend 都通过 compose 启动）
...
启动顺序（从零开始）：**源码与依赖 → config.yml / .env → 启动 compose 依赖（PostgreSQL + Redis）→ 模型 → OpenClaw（可选）→ Telegram Bot 凭据 → 启动 bot / backend 容器 → 宿主机上启动 core 引擎**。
...
### 3. 启动 Redis + PostgreSQL（Docker Compose）
...
OpenClaw 是 Seedwake 用来**派发非原生动作**的执行层。下面这些动作都要通过 OpenClaw worker agent 执行：
...
要让 HTTP fallback 在 WS 断线时自动接管，需要同时设置 `actions.use_openclaw_http_fallback: true`：
...
**Bot token**：在 [@BotFather](https://t.me/BotFather) 处创建一个 Bot，拿到 token，填入 `.env`。如果同时要启动 backend，请一并把 `BACKEND_API_TOKEN` 也填上：
...
在启动 core 之前，建议先跑一次完整测试套件确认环境没问题：
...
- **core**：`Ctrl+C` 发送 `SIGINT`，core 会把当前 cycle 跑完、清空动作队列、关闭 Redis / PostgreSQL 连接后退出。**不要用 `kill -9`**，会丢失尚未持久化的短期记忆和行动状态。
- **bot / backend / 依赖**：`docker compose down` 会停掉所有容器。如果只是想停容器但保留数据卷（`data/postgresql`、`data/redis`），**不要**加 `-v`。
...
暂未开放公开 PR，以 GitHub Issue 为主要贡献入口。若您有长期稳定积极参与的意向，欢迎您成为 collaborator，直接提交代码。

**Bug**：请描述现象、复现步骤、期望行为、相关日志（`data/logs/` 下的片段，敏感内容请自行脱敏）、运行环境。

**新功能**：请写清楚想解决什么问题、为什么是这个方案。若您已在本地实现过，请一并附上您用的 prompt、踩过的坑、验收标准。您不需要亲自提交代码，维护者会根据 issue 在本地实现、review、合入。

**运行教程**：「运行方式」和「配置与部署」两节可能有遗漏。您在部署时若遇到问题，请先在本地咨询 Claude Code 或 Codex；确认是教程本身有误（描述错、步骤漏、命令过时），请再提 issue。
