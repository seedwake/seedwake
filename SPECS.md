...
[7]  我的笔记                  （持久草稿纸，如有内容）
[8]  好像有一阵子没有……        （感知提醒，如有）
[9]  最近的念头                （从 Redis，最近 N 轮 × 3 条）
[10] 浮上来的记忆              （念头触发的向量检索 top-k）
[11] 行动有了回音              （当前 + 近期 echo 缓存）
[12] 我已经发起、在等确认的事  （pending actions）
[13] 我已经发起、正在等回音的事（running actions）
[14] 此刻我注意到              （被动感知：时间、系统状态等）
[15] 对当前对话者的印象        （impression，如有匹配 entity）
[16] 最近的对话                （对话历史 + LLM 滚动摘要）
[17] 有人对我说话了            （当前轮新消息）
[18] 停滞警告                  （如检测到退化）
[19] 接下来的念头 --- 第 N 轮 ---（生成点）
```
