...
--- 念头流 + 联想 ---
[2]  最近的念头                （从 Redis，最近 N 轮 × 3 条）
[3]  浮上来的记忆              （念头触发的向量检索 top-k）
--- 内部状态（条件渲染，大部分轮次不出现）---
[4]  此刻需要留意              （前额叶：定期检查轮或退化干预时）
[5]  此刻的自我感              （末那识：仅当有 session_context/warning/identity_notice）
[6]  此刻的情绪                （仅当某维度强度 ≥ 0.65）
[7]  最近的反思                （元认知反思，如有，去重）
--- 背景 ---
[8]  我的笔记                  （持久草稿纸，如有内容）
[9]  好像有一阵子没有……        （感知提醒，如有）
...
[16] 刚才还在继续的对话        （reply_focus：无新消息但 10 分钟内有对话时）
[17] 有人对我说话了            （当前轮新消息）
--- 控制 ---
[18] 停滞警告                  （如检测到停滞）
[19] 这一轮的硬约束            （退化干预时，紧贴生成点）
[20] 接下来的念头 --- 第 N 轮 ---（生成点）
```
